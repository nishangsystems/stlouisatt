...
$select =$con->query("SELECT * FROM   courses,prog_courses WHERE courses.id=prog_courses.course_id
AND prog_courses.id='".$_GET['id']."'  ") or die(mysqli_error($con));
...
while($rows=$select->fetch_assoc()){
 ...
 ?> 
 <?php echo $course_title; ?> (<?php echo $course_code; ?>) Course Report for <?php echo $prog_name;  ?> <?php echo $level_name; ?> <?php echo $sem_name; ?>  for <?php echo $ayear_name; ?>
 <br>
 Campus: <?php echo $camp_name; ?> Campus
 <?php 


 $check_exits=$con->query("SELECT * FROM courses,prog_courses WHERE courses.id=prog_courses.course_id
 and prog_courses.id='".$_GET['id']."'  AND prog_courses.level_id='$level_id' AND prog_courses.prog_id='$prog_id' 
  ") 
...
AND departure IS NOT NULL AND prog_courses.id='".$rows['id']."'")  or die(mysqli_error($con));
  ...
  <h2>LECTURERS IN THIS COURSE AND THEIR RECORDS</h2>
 ...
 $check_exits=$con->query("SELECT * FROM courses,users, prog_courses,teacher_courses WHERE courses.id=prog_courses.course_id
 and prog_courses.id='".$_GET['id']."' AND teacher_courses.course_id=prog_courses.id
 and teacher_courses.year_id='$year_id' AND users.id=teacher_courses.teacher_id  AND prog_courses.level_id='$level_id' AND prog_courses.prog_id='$prog_id' 
  ") 
        or die(mysqli_error($con));
        $i=1;
         $nums=$check_exits->num_rows;
...
<td><a href="?course_records&year_id=<?php echo $year_id; ?>&course_id=<?php echo $rows['course_id']; ?>&id=<?php echo $rows['teacher_id']; ?>&link=<?php echo $rows['full_name']; ?> Attendance Records for <?php echo $rows['course_title']; ?> (<?php echo $rows['course_code']; ?>)">
<?php echo $rows['full_name']; ?>  </td>


<td><?php $query=$con->query("SELECT * FROM   teacher_courses  where 
teacher_courses.teacher_id='".$rows['teacher_id']."' AND 
teacher_courses.course_id='".$rows['course_id']."' and teacher_courses.year_id='$year_id'") 
        or die(mysqli_error($con));
        

   while($results=$query->fetch_assoc()){
     echo  $lectures=$results['lectures'];
   } ?>   </td>
<td><?php 
$query=$con->query("SELECT * FROM   teacher_courses  where 
teacher_courses.teacher_id='".$rows['teacher_id']."' AND 
teacher_courses.course_id='".$rows['course_id']."' and teacher_courses.year_id='$year_id'") 
        or die(mysqli_error($con));
   ...
   while($results=$query->fetch_assoc()){
     echo  $tutorials=$results['tutorial'];
   }
?> </td>
<td><?php $query=$con->query("SELECT * FROM   teacher_courses  where 
teacher_courses.teacher_id='".$rows['teacher_id']."' AND 
teacher_courses.course_id='".$rows['course_id']."' and teacher_courses.year_id='$year_id'") 
   ...
   while($results=$query->fetch_assoc()){
     echo  $practical=$results['practicals'];
   } ?>   </td>
<td><?php 
...
AND topics.course_id='".$rows['course_id']."'
 ") or die(mysqli_error($con));
 $total_lessons= $select->num_rows;
...
where 
 prog_courses.id=topics.course_id AND topics.id=sub_topics.topic_id AND sub_topics_taught.year_id='$year_id'
 and sub_topics_taught.subtopic_id=sub_topics.id AND  prog_courses.id='".$rows['course_id']."'
 AND sub_topics_taught.teacher_id='".$rows['teacher_id']."'
 ") or die(mysqli_error($con));
...
$check_all=$con->query("SELECT SUM(TIMESTAMPDIFF(HOUR,  arrival,departure)) AS tim
FROM prog_courses,teacher_att WHERE prog_courses.id=teacher_att.course_id AND 
    teacher_att.year_id='$year_id' AND teacher_att.teacher_id='".$rows['teacher_id']."'
AND departure IS NOT NULL AND prog_courses.id='".$rows['course_id']."'")  or die(mysqli_error($con));
      while($ro=$check_all->fetch_assoc()){
          $total_hrs_used= $ro['tim'];
      }  
      
      if($total_hrs_used>$lectures){
       echo "<span style='color:#fff;padding:10px 20px;background:#F00;font-weight:bold'>".$total_hrs_used."</span>";
   }
   else {
     echo  $total_hrs_used;
