...
<script>tinymce.init({selector:'textarea'});</script>
 ...
 $check=$con->query("SELECT * FROM  topics where id='$tid' ") 
       ...
       $check=$con->query("SELECT * FROM  sub_topics where id='$id' ") 
       or die(mysqli_error($con));
      
       while($rows=$check->fetch_assoc()){
           $topic_sub=$rows['content'];
       }
       ?>
       </u></span><br>
     SUB TOPIC : <span style="color:#F00; font-style:italic; text-transform:capitalize"><?php echo $topic_sub; ?></span>
...
WHERE   topics.id='$tid'  AND teacher_att.year_id='$year_id' AND teacher_att.id='$att_id'
 ...
 <table class="table .table-bordered" style="font-size:14px">
	...
	?>
      ...
      <?php }  ?>
         ...
         <?php } ?>
         <?php SaveLogBook($course_id,$att_id,$id,$tid); ?>
