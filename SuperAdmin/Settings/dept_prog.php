						...
						if(isset($_GET['id'])){
							$select =$con->query("SELECT * FROM  departments where id='".$_GET['id']."'  ") or die(mysqli_error($con));
						...
						<form method="POST" action="" class="form-horizontal" role="form">
					       	...
					       	<div class="form-group">
										...
										<div class="col-sm-9">
											<input type="text" value="<?php echo $rows['dept_name']; ?>"   id="form-field-1"   class="col-xs-10 col-sm-5" />
										</div>
									</div>
									<div class="form-group">
                         ...
                         <table class="table table-bordered">
                            ...
                            <tbody>
                            <?php
							$select =$con->query("SELECT * FROM  programs,department_programs WHERE  department_programs.dept_id='".$_GET['id']."'
							AND programs.id=department_programs.prog_id order by department_programs.id DESC  ") or die(mysqli_error($con));
							$a=1;	
							while($rows=$select->fetch_assoc()){
							?>
                              <tr>
                                ...
                                <td><a href="?create_dept_prog&id=<?php echo $rows['dept_id']; ?>&del=<?php echo $rows['id']; ?>&888d8d8" class="btn btn-danger btn-xs">Delete</button></td>
                              </tr>
                             <?php } ?>
                            </tbody>
                      </table>
                      <?php 	}
						...
						if(isset($_GET['del'])){
						echo	$id=$_GET['del'];
						$select =$con->query("DELETE  FROM  department_programs WHERE  id='$id' ") or die(mysqli_error($con));
						echo "<script>alert('Successfully Deleted. Thank you')</script>";
				echo '<meta http-equiv="Refresh" content="0; url=?create_dept_prog&id='.$_GET['id'].'">';
