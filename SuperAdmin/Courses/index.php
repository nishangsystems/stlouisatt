               ...
               <form method="POST" action="?confirm_it" enctype="multipart/form-data">
                   ...
                   <div class="form-row">
                       ...
                       <label for="inputPassword4"> Teacher :
                           </label>
                           <select  name="user_id" onBlur="doCalc(this.form)" required class="form-control">
                               <option    onBlur="doCalc(this.form)"></option>
                               <?php
                               $select =$con->query("SELECT * FROM   users WHERE user_level='2' ORDER BY full_name") or die(mysqli_error($con));
                           
                               while($rows=$select->fetch_assoc()){
                                   ?>
                               <option value="<?php echo $rows['id'] ?>"  onBlur="doCalc(this.form)"><?php echo $rows['full_name'] ?></option>
                               <?php } ?>
